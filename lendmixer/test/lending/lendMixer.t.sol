...
contract LendMixerTest is Test {
    ...
    function testFlashLoanSuccess() public {
        ...
        vm.prank(user);
        bool success =
            ILendMixer(address(proxy)).flashLoan(address(receiver), address(token), 1e18, abi.encode(true, true));
    ...
    function testFlashLoanIsNotContract() public {
        vm.prank(user);
        vm.expectRevert();
        ILendMixer(address(proxy)).flashLoan(user, address(token), 0, abi.encode(true, true));
    }
    ...
    function testFlashLoanTokenNotSupport() public {
        ...
        vm.prank(user);
        vm.expectRevert();
        ILendMixer(address(proxy)).flashLoan(address(receiver), address(token), 0, abi.encode(true, true));
    }
    ...
    function testFlashLoanAmountNotEnough() public {
        ...
        vm.prank(user);
        vm.expectRevert();
        ILendMixer(address(proxy)).flashLoan(address(receiver), address(token), 1e21, abi.encode(true, true));
    }
    ...
    function testFlashLoanRepayAmountNotEnough() public {
        ...
        vm.prank(user);
        vm.expectRevert();
        ILendMixer(address(proxy)).flashLoan(address(receiver), address(token), 1e18, abi.encode(false, true));
    }
    ...
    function testFlashLoanReturnValueInvalid() public {
        ...
        vm.prank(user);
        vm.expectRevert();
        ILendMixer(address(proxy)).flashLoan(address(receiver), address(token), 1e18, abi.encode(true, false));
    }
